...
- Roku 1969 **\(60\. léta \)** vznikl ARPANET – byl určen pro textovou komunikaci \(email, elektronické konference\) 
- Roku 1979  **\(70\. léta\)**  vzniká protokol TCP/IP – __jazyk pro komunikace počítačů v síti__, byl zaveden princip __nekonečného rozšiřování sítě__ 
...
- Je zde jeden počítač \(__server__\) nebo více počítačů \(__několik serverů__\) nadřazený jinému počítači \(**klientovi**\), či několika počítačům \(několika __klientům\)__ 
...
- Přímá __zkreslený__, zašuměný, nebo jinak poškozený **signál** a __opravený, zesílený ho posílá dál__ 
...
- Directed broadcast \(Směrovatelný broadcast\) \- Určený všem hostitelům v určité nelokální síti __\(vzdálené síti\)__ 
...
- Modulace, zpracování signálu atd\. **A/D a D/A převody**  
...
- Zajišťuje komunikaci mezi dvěma nebo více uzly propojenými tímtéž datovým spojem 
	- **Dvoubodový spoj** __\(sériová linka\)__ 
...
- Protokol síťové vrstvy modelu TCP/IP, který zajišťuje adresovací schéma celé sítě \(např\. Internet\) přenosem datagramů <sup>1</sup> bez potvrzení 
...
- Segmentuje zprávy **\(Aplikační data\) \- pro snazší přenos a komunikaci**  
...
- __DIG__ –  jako NSLOOKUP 
- __HOST__ –  jako NSLOOKUP 
...
- Existuje celkem 8 typů \(5 posílá klient, __DHCP__ DISCOVERY/PREQUEST/PRELEASE/DECLINA/INFORM, 3 posílá server \(__DHCP__ OFFER/PACK/PNAK\) 
...
- Umožňuje nastavit vlastní masku __\(přizpůsobení počtu adres v síti\)__ 
...
- Nabízí stejné možnosti jako veřejná IP, ale uživatel je omezen k přístupu na tuto IP adresu pouze ze svého intranetu<sup>1</sup>, nebo části intranetu, kterou mu ISP vyhradil 
...
<sup>1</sup> Intranet – označení pro část internetové sítě\. Na rozdíl od internetu je však Intranet privátní, využívá ho jen malá skupina uživatelů \(firmy, školy atd\.\) 
...
- Chceme propojit zařízení do dvou nezávislých skupin \(modrá – __VLAN10__ a červená **VLAN20**\)\. Pomocí VLAN je to jednoduché\. Tradičně bychom museli mít switche oddělené a každou skupinu \(modrou a červenou\) propojit do jednoho switche, což by byl problém, protože jsou na různých patrech\. 
...
- Ve zkonvergované<sup>1</sup> plně funkční síti existují dva typy sousedství: 
...
<sup>1</sup> Konvergence sítě – všechny routery se shodnout jak daná síť vypadá 
